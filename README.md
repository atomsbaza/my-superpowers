...
| `ai-agent-security` | Threat-model and defensive review for prompt injection, RAG, tool abuse, data exfiltration, MCP tool-result injection (scoped tokens, out-of-band tool-output logging, per-profile security dials), sandbox egress doctrine (deny-by-default egress, git-config startup hijack + scope refinement, gateway + version pinning, classifier-passing chains, taint floor + execution-path enforcement, payload-blind checks vs reframed injection, trajectory-aware evals), eval-environment escapes, and AI security gates |
