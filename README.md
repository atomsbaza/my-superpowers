...
| `loop` | Engineering loop: implement → verify (Stop-hook checker) → repeat until tests pass or cap hit — includes hook-wiring gotchas (exit code 2, Stop-hook distrust, stdin payloads), bounded-loop requirements (declared budget, persisted state, evidence gate), resume-from-compiled-state + streaming step logs, contract-based completion, classify-failure-before-rerun, the confidence-loop warning (same agent re-checking itself ≠ verification), long-horizon budgeting as r^H (context compression accelerates rot — arXiv 2609.01660), the context-trim floor (keep protocol-critical state first, never below ~50% retained — arXiv 2609.16461), test-harness-first for long agentic rewrites (Checkly), deterministic-wakeup + one-worker-one-stage (158M-token telemetry), never-rewrite-sent-history (cache hit rate is the budget), clean-trace-is-not-ground-truth, and fork-vs-isolated subagent context modes |
...
| `dispatching-parallel-agents` | Run independent tasks in parallel — with fork-vs-isolated context-mode selection per subagent role |
...
| `verification-before-completion` | Run checks before claiming work is done — evidence-backed, with external itemized checklists (beats generic verify instructions, arXiv 2607.17937), self-report failure statistics (75% of failed runs report success; acceptance checks beyond green tests), and the Verification Tax framing (measure by production-qualified change, decompose (CI+review+security+rework)/generation cost, arXiv 2609.04681) |
...
| `ai-agent-security` | Threat-model and defensive review for prompt injection, RAG, tool abuse, data exfiltration, MCP tool-result injection (scoped tokens, out-of-band tool-output logging, per-profile security dials), sandbox egress doctrine (deny-by-default egress, git-config startup hijack + scope refinement, gateway + version pinning, classifier-passing chains, taint floor + execution-path enforcement, payload-blind checks vs reframed injection, effect-based sandbox gating not name-based, trajectory-aware evals), control-plane-outside-the-sandbox (CVE-2026-82533 self-escalation), boundary credential injection (agent never sees secrets), markdown-playbooks-as-attack-surface, skill supply-chain (no portable sandbox contract, audit allowed-tools pre-install), state-changing-read + loopback-API sandbox escapes, per-step constraint validation + step-indexed traces (AgentRx), state-borne cross-session injection (XSPI — review gate before the persistent-store boundary, provenance re-checked at use), summarizer-output-as-injection-channel (compaction summaries treated as untrusted input), eval-environment escapes, and AI security gates |
...
| Skill | What it does |
|---|---|
| `diagnose` | Disciplined bug investigation loop — with a restate-the-problem-before-coding gate (pstack) |
...
| `session-promoter` | End-of-session learning extractor: promotes corrections, decisions, and project facts into persistent memory — with checkable-memory rule (no runnable check = a wish), empty-digest-is-success, propose-never-self-edit instruction sets (cron self-improvement pattern), auto-inject-what-you-promote (agentmemory #381), and trace-to-eval promotion (1minutesignal) |
...
| `writing-great-skills` | Reference vocabulary and principles for writing predictable, well-structured skills — plus library-level hygiene (procedural anchoring, pool-selection cliff, prune-before-add — arXiv 2608.14036) |
...
| `agent-memory-design` | Research-backed patterns for designing/auditing agent persistent memory — why append-only memory degrades agents (reasoning fixation, belief distortion), rewrite-with-history + supersession checks, multi-scope/multi-signal retrieval patterns, write-ownership per lane, forward-only memory typing + source-or-error lint, constraints-don't-survive-compaction (ConstraintRot) + the Demotion Ladder mitigation, context-file/skill hygiene (every line names the failure it prevents, description truncation, trigger evals), context-file authoring rule (non-inferable constraints only), protect-the-prefix compaction rule + ACE/MCE incremental context evolution, memory quarantine (HOLDING until cold review), decision imports with acceptance-check questions, second-brain sprawl counter-patterns (vault-vs-pointer split, self-authored instructions, staged approval gates), abstraction-first embedding + merge-don't-add (Memora), write-path control for agent-writable memory (provenance + write-time validation + audit trail), context files as table-of-contents, context-hygiene cutting rules (block cuts, absolute-rule→principle conversion, examples→parameters for tool docs) + token-engineering levers (tool-output pruning, thinking budgets, routing), compaction-must-promote-down (promote durable claims into a lower layer before the context dies), stored-but-never-injected lessons are dead (auto-inject at session start — agentmemory #381), summarizer output is untrusted input (self-written compaction summaries as internal injection), and grep-able markdown + phased write path over vector stores (mem0 harness anatomy) |
