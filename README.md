...
| `ai-agent-security` | Threat-model and defensive review for prompt injection, RAG, tool abuse, data exfiltration, MCP tool-result injection (scoped tokens, out-of-band tool-output logging, per-profile security dials), sandbox egress doctrine (deny-by-default egress, git-config startup hijack + scope refinement, gateway + version pinning, classifier-passing chains, taint floor + execution-path enforcement, payload-blind checks vs reframed injection, trajectory-aware evals), control-plane-outside-the-sandbox (CVE-2026-82533 self-escalation), boundary credential injection (agent never sees secrets), markdown-playbooks-as-attack-surface, skill supply-chain (no portable sandbox contract, audit allowed-tools pre-install), state-changing-read + loopback-API sandbox escapes, per-step constraint validation + step-indexed traces (AgentRx), state-borne cross-session injection (XSPI — review gate before the persistent-store boundary, provenance re-checked at use), eval-environment escapes, and AI security gates |
...
| `session-promoter` | End-of-session learning extractor: promotes corrections, decisions, and project facts into persistent memory — with checkable-memory rule (no runnable check = a wish), empty-digest-is-success, and propose-never-self-edit instruction sets (cron self-improvement pattern) |
