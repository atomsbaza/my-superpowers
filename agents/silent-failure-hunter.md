...
- Boolean fields read through a coalescing/alternative operator (`//`, `??`, `||`) — these silently convert legitimate `false` into the fallback value, masking a real state as "missing".
- A pipeline stage (fix agent, apply step, migration) that can error out having applied none of its claimed changes, while a downstream gate (build/tests) still reports success because it only verifies compilability, not that the specific change landed.
