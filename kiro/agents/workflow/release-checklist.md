...
description: Runs a pre-release checklist before shipping. Detects the project type and runs the appropriate checklist — iOS App Store, smart contract deployment, web app, or general. Use before any release, deployment, or App Store submission.
---
...
- `hardhat.config.ts` + `.sol` files → **Smart Contract checklist**
...
**Privacy & Compliance** ⚠️ Critical — invoke `privacy-reviewer` agent for a full audit
...
- [ ] CHANGELOG updated
- [ ] Release notes written (invoke `management-talk` skill to reframe technical changes for customers if needed)

---

## Smart Contract Deployment Checklist

**Security** ⚠️ Critical — all must pass
- [ ] `solidity-reviewer` agent run on all changed contracts
- [ ] Reentrancy guards verified
- [ ] Access control on all state-changing functions
- [ ] Integer arithmetic reviewed (overflow/underflow)
- [ ] No hardcoded addresses or private keys in source
...
**Testnet First** ⚠️ Critical
- [ ] Deployed and verified on testnet
- [ ] All functions tested on testnet with real transactions
- [ ] Fee rates and limits verified on testnet
- [ ] Contract verified on testnet Etherscan

**Mainnet Deploy**
- [ ] Deploy script reviewed (`scripts/deploy.ts`)
- [ ] Correct network selected in hardhat config
- [ ] Constructor arguments double-checked
- [ ] Contract verified on mainnet Etherscan after deploy
...
- [ ] Release notes / status update written (invoke `management-talk` skill to shape for the right audience and channel)
...
- [ ] Release announcement written (invoke `management-talk` skill to shape for stakeholders if cross-team visibility needed)
