...
```bash
#!/bin/bash
# Save this as release.sh in your own repo's scripts/ directory.
...
### Before writing CHANGELOG.md to disk
Spawn `release-checklist` first. If it returns any **Critical** blocker:
- Do NOT write CHANGELOG.md to disk
- Surface blockers to the user and stop
- Only resume after the user resolves or explicitly overrides each blocker

Once checklist is clean (or nit-only), write CHANGELOG.md, then spawn `doc-updater` to check if any docs reference version numbers, feature flags, deprecated APIs, or migration paths that need updating. Pass it the changelog diff and the new version number.

> **Before spawning:** Skip if no git tag exists yet (pre-release / first version). If release-checklist errors or returns empty, treat it as unresolved — do not write the changelog. If doc-updater errors, note it explicitly.
