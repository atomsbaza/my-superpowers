...
- **For regression tests after a bug fix:** after invoking `debugger` and applying a fix, write a test that exercises the exact code path the bug touched. This becomes the regression lock to prevent the same bug class from slipping through again. (See `post-mortem` skill for documenting the fix itself.)
